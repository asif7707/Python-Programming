...
def new():
    ...
    btn = Button(top, text='Open python file', command=lambda: open_file())
    btn.grid()
...
def submit():
    ...
    if name=='asif' and password=='asif123':
        new()

    else:
        if name!='asif' or password!='asif123':
            messagebox.showerror('wrong','wrong id\n please correcting your info')
...
# creating a label for
# name using widget Label
name_label = tk.Label(windo, text='Username',
                      font=('calibre',
                            10, 'bold'))

# creating a entry for input
# name using widget Entry
name_entry = tk.Entry(windo,
                      textvariable=name_var, font = ('calibre', 10, 'normal'))

# creating a label for password
passw_label = tk.Label(windo,
                       text='Password',
                       font=('calibre', 10, 'bold'))

# creating a entry for password
passw_entry = tk.Entry(windo,
                       textvariable=passw_var,
                       font=('calibre', 10, 'normal'),
                       show='*')
...
# creating a button using the widget
# Button that will call the submit function
sub_btn = tk.Button(windo, text='Log In',
                    command=submit)
